...
local ITEM = catherine.item.New( "health_vial" )
ITEM.name = "^Item_Name_HealthV"
ITEM.desc = "^Item_Desc_HealthV"
ITEM.cost = 100
...
ITEM.func.use = {
	...
	func = function( pl, itemTable, ent )
		if ( pl:Alive( ) ) then
			local healAmount = hook.Run( "GetHealAmount", pl, itemTable ) or 15
...
ITEM.func.heal = {
	...
	func = function( pl, itemTable )
		...
		if ( IsValid( ent ) and ent:IsPlayer( ) ) then
			if ( ent:Alive( ) ) then
				local healAmount = hook.Run( "GetHealAmount", ent, itemTable ) or 15
